...
#### 2. madpack 실행.madpack utility <br>
-- 사전에 plpython3u 확장 기능 구성.
```
create extension plpython3u; 
```

-- change to Python2 env at Master/Coordinator <br>
-- only WarehousePG 6
```
...
### 1. 선형 회귀 모델 학습 및 예측 (madlib.linregr) <br>
...
#### 1-3. 모델 생성 (madlib.linregr_train) <br>
선형 회귀 모델 함수를 사용해 regr_example_model 테이블에 회귀 모델을 생성합니다. <br>
```
...
#### 1-5 예측 및 잔차(residual) 계산 <br>
선형 회귀 모델 함수를 이용해 실제값(y)과 예측값(predict)의 차이를 확인합니다. <br>
```
SELECT regr_example.*, 
...
```
### 2. 연관 규칙 분석 (madlib.assoc_rules) <br>

#### 2-1. 테스트 테이블 생성 및 데이터 삽입 <br>
```
...
### 3. 나이브 베이즈 분류 (Naive Bayes) 예제 1 — 단순 숫자형 속성 <br>

#### 3-1. 테이블 생성 및 데이터 삽입 <br>
실제 환경에서는 더 많은 학습 데이터가 있을수록 모델 정확도가 향상됩니다. <br>
```
...
#### 3-4. 분류 결과 뷰 생성 <br>
madlib.create_nb_probs_view() 함수로 예측 결과를 계산할 뷰를 생성합니다. <br>
```
...
### 4. 나이브 베이즈 분류 (Naive Bayes) 예제 2 — 날씨 데이터 <br>

#### 4-1 테이블 생성 및 데이터 삽입<br>
```
